...
int main(int argc, char** argv) {
  ...
  dim3 ptx_mma_oc32_dual_n_tile5_grid((POOL_OW + 4) / 5, (POOL_OH + 4) / 5,
                                      OC / 32);
  float ptx_mma_oc32_dual_n_tile5_ms = time_kernel([&] {
    fused_ptx_mma_oc32_dual_n_smem_b_kernel<5, 4>
        <<<ptx_mma_oc32_dual_n_tile5_grid, 128>>>(d_x, d_w_mma, d_y, shift);
  }, args.warmup, args.iters);
  CUDA_CHECK(cudaMemcpy(h_y.data(), d_y, y_count, cudaMemcpyDeviceToHost));
  print_result(args, "ptx_mma_oc32_dual_n_5x5_w4_b128",
               ptx_mma_oc32_dual_n_tile5_ms, max_abs_err(h_ref, h_y));

  dim3 ptx_mma_oc32_dual_n_tile6_grid((POOL_OW + 5) / 6, (POOL_OH + 5) / 6,
                                      OC / 32);
  float ptx_mma_oc32_dual_n_tile6_ms = time_kernel([&] {
    fused_ptx_mma_oc32_dual_n_smem_b_kernel<6, 4>
        <<<ptx_mma_oc32_dual_n_tile6_grid, 128>>>(d_x, d_w_mma, d_y, shift);
  }, args.warmup, args.iters);
  CUDA_CHECK(cudaMemcpy(h_y.data(), d_y, y_count, cudaMemcpyDeviceToHost));
  print_result(args, "ptx_mma_oc32_dual_n_6x6_w4_b128",
               ptx_mma_oc32_dual_n_tile6_ms, max_abs_err(h_ref, h_y));

  dim3 ptx_mma_oc32_dual_n_tile7_grid((POOL_OW + 6) / 7, (POOL_OH + 6) / 7,
                                      OC / 32);
  float ptx_mma_oc32_dual_n_tile7_ms = time_kernel([&] {
    fused_ptx_mma_oc32_dual_n_smem_b_kernel<7, 4>
        <<<ptx_mma_oc32_dual_n_tile7_grid, 128>>>(d_x, d_w_mma, d_y, shift);
  }, args.warmup, args.iters);
  CUDA_CHECK(cudaMemcpy(h_y.data(), d_y, y_count, cudaMemcpyDeviceToHost));
  print_result(args, "ptx_mma_oc32_dual_n_7x7_w4_b128",
               ptx_mma_oc32_dual_n_tile7_ms, max_abs_err(h_ref, h_y));
