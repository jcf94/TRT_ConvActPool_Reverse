...
// v39: wide-tile S2 step toward TRT 8x120. Dynamic shared memory unlocks large
...
__global__ void fused_oc64_i8vpool_kernel(
...
#pragma unroll
        for (int og = 0; og < OC_GROUPS; ++og) {
          ...
          if (n0 < N_TOTAL) conv_acc[n0 * OC_PER_BLOCK + c] = clamp_relu_i8(acc[0][og][i], shift);
          if (has_n1 && n1 < N_TOTAL) conv_acc[n1 * OC_PER_BLOCK + c] = clamp_relu_i8(acc[1][og][i], shift);
        }
      }
    }
  }
...
#pragma unroll
          for (int c = 0; c < OC_PER_BLOCK; ++c)
            best[c] = max(best[c], int(conv_acc[sidx * OC_PER_BLOCK + c]));
        }
      }
...
static void run_case(const Args& args, const char* name, const int8_t* d_x,
  ...
  auto k = fused_oc64_i8vpool_kernel<TILE, WARPS, OC_GROUPS>;
...
int main(int argc, char** argv) {
  ...
  run_case<8, 8, 4, 256>(args, "v39_oc64_wide_pool_8x8_b256", d_x, d_w_mma4, d_y, shift, h_ref, h_y);
  run_case<7, 8, 4, 256>(args, "v39_oc64_wide_pool_7x7_b256", d_x, d_w_mma4, d_y, shift, h_ref, h_y);
  run_case<4, 8, 4, 256>(args, "v39_oc64_wide_pool_4x4_b256", d_x, d_w_mma4, d_y, shift, h_ref, h_y);
