...
__global__ void fused_ptx_mma_oc32_delayed_relu_pool_kernel(
...
#pragma unroll
        for (int which = 0; which < 2; ++which) {
          ...
          if ((which == 0 || has_n1) && n < N_TOTAL) {
            int conv_x = n % CONV_TILE;
            int conv_y = n / CONV_TILE;
            int lx0 = max(0, (conv_x - 1) / 2);
            int ly0 = max(0, (conv_y - 1) / 2);
...
#pragma unroll
                  for (int og = 0; og < OC_GROUPS; ++og) {
                    ...
                    int v = which ? acc[1][og][i] : acc[0][og][i];
  ...
  for (int i = tid; i < OC_PER_BLOCK * POOL_COUNT; i += blockDim.x) {
    ...
    if (px < POOL_OW && py < POOL_OH) {
      y[(oc_base + c) * POOL_OH * POOL_OW + py * POOL_OW + px] =
          clamp_relu_i8(pool_accum[c][p], shift);
    }
  }
}
...
static void run_case(const Args& args, const char* name, const int8_t* d_x,
  ...
  float ms = time_kernel([&] {
    fused_ptx_mma_oc32_delayed_relu_pool_kernel<TILE, WARPS>
...
int main(int argc, char** argv) {
  ...
  run_case<4, 4, 128>(args, "ptx_mma_oc32_delayed_relu_pool_4x4_w4_b128", d_x,
                      d_w_mma4, d_y, shift, h_ref, h_y);
  run_case<4, 8, 256>(args, "ptx_mma_oc32_delayed_relu_pool_4x4_w8_b256", d_x,
                      d_w_mma4, d_y, shift, h_ref, h_y);
