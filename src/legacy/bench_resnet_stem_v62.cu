...
__global__ void v62_kernel(const int8_t* __restrict__ x,const uint32_t* __restrict__ w,int8_t* __restrict__ y,int shift){
 ...
 for(int ng=warp;ng<NG;ng+=12){int32_t acc[OCG][4]={};
...
int main(int argc,char**argv){Args a=parse_args(argc,argv);constexpr int sh=9;
 ...
 float ms=time_kernel([&]{v62_kernel<<<g,384>>>(dx,dw,dy,sh);},a.warmup,a.iters);
 CUDA_CHECK(cudaMemcpy(hy.data(),dy,hy.size(),cudaMemcpyDeviceToHost));print_result(a,"v62_12warp",ms,max_abs_err(hr_n,hy));
