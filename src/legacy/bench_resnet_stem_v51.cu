...
// v51: keep 240 IMMA, cut REG/STG. NG-outer loop frees acc[4][4] each column so
...
__global__ void v51_kernel(const int8_t* __restrict__ x, const uint32_t* __restrict__ w,
  ...
  // halo tile: 8x12 conv -> interior PB_HxPB_W pool, stepped by PB to cover all
  constexpr int GX = (POOL_OW + PB_W - 1) / PB_W;
  int gx0 = (blockIdx.x % GX) * PB_W, gy0 = blockIdx.x / GX * PB_H;
  int bx = gx0 * 2 - 1, by = gy0 * 2 - 1;
...
#pragma unroll
        for (int og = 0; og < OCG; ++og)
          ((int8_t*)cr)[n * 64 + og * 16 + row] = clamp_relu_i8(acc[og][i], shift); }
    }
  __syncthreads();
...
int main(int argc, char** argv) {
  ...
  float ms=time_kernel([&]{v51_kernel<<<g,256>>>(dx,dw,dy,sh);},a.warmup,a.iters);
  CUDA_CHECK(cudaMemcpy(hy.data(),dy,hy.size(),cudaMemcpyDeviceToHost));
  print_result(a,"v51_240imma_pool_8x12",ms,max_abs_err(hr,hy));
