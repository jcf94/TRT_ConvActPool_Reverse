...
__global__ void v66_kernel(const uint32_t* __restrict__ b, const uint32_t* __restrict__ w,
                           int8_t* __restrict__ y, int shift) {
  __shared__ uint32_t bb[2][N_TILE * KC];
  uint32_t* cr = &bb[0][0];  // alias pool tile onto B-stage smem (reused after mainloop)
  ...
  // each thread cp.async-loads one n*16B (4 uint32) row segment of the chunk
  
  for (int i = tid; i < N_TILE * 2; i += blockDim.x) { int n = i >> 1, h = i & 1;
    int gn = (by + n / CB_W) * CONV_OW + (bx + n % CB_W);
    cpasync16((uint32_t)__cvta_generic_to_shared(&bb[0][n*KC+h*4]), &b[gn * K_GROUPS_MMA + 0 * KC + h * 4]); }
  asm volatile("cp.async.commit_group;\n" ::); asm volatile("cp.async.wait_all;\n" ::); __syncthreads();
  for (int ch = 0; ch < NCH; ++ch) { int cur = ch & 1;
    if (ch + 1 < NCH) { int nx = ch + 1;
      for (int i = tid; i < N_TILE * 2; i += blockDim.x) { int n = i >> 1, h = i & 1;
        int gn = (by + n / CB_W) * CONV_OW + (bx + n % CB_W);
        cpasync16((uint32_t)__cvta_generic_to_shared(&bb[nx&1][n*KC+h*4]), &b[gn * K_GROUPS_MMA + nx * KC + h * 4]); }
      asm volatile("cp.async.commit_group;\n" ::); }
#pragma unroll
...
#pragma unroll
    ...
    if (ch + 1 < NCH) { asm volatile("cp.async.wait_all;\n" ::); }
    __syncthreads();
  }
#pragma unroll
...
int main(int argc, char** argv) {
  ...
  float ms=time_kernel([&]{v66_kernel<<<g,WARPS*32>>>(db,dw,dy,sh);},a.warmup,a.iters);
  CUDA_CHECK(cudaMemcpy(hy.data(),dy,hy.size(),cudaMemcpyDeviceToHost));
  print_result(a,"v66_union",ms,max_abs_err(hr_n,hy));
