...
__global__ void v58_kernel(const int8_t* __restrict__ x,const uint32_t* __restrict__ w,int8_t* __restrict__ y,int shift){
 __shared__ uint32_t patch[PH*PW]; __shared__ uint32_t cr[N_TILE*16];
 ...
 for(int i=threadIdx.x;i<PH*PW;i+=blockDim.x){int iy=oy+i/PW,ix=ox+i%PW;patch[i]=(iy>=0&&iy<IH&&ix>=0&&ix<IW)?((const uint32_t*)x)[iy*IW+ix]:0;}
 __syncthreads();
 ...
 for(int ng=warp;ng<NG;ng+=6){int32_t acc[OCG][4]={};
  for(int kb=0;kb<K_PAD_MMA;kb+=32){int kgb=kb>>2;uint32_t b[2];
   ...
   for(int br=0;br<2;++br){int n=ng*8+gid,cx=n%CB_W,cy=n/CB_W,v[4];
    #pragma unroll
    for(int e=0;e<4;++e){int k=(kgb+lig+br*4)*4+e;v[e]=0;if(k<K_TOTAL){int ic=k/49,py=cy*2+(k/7)%7,px=cx*2+k%7;v[e]=((const int8_t*)patch)[(py*PW+px)*4+ic];}}
...
int main(int argc,char**argv){Args a=parse_args(argc,argv);constexpr int sh=9;
 ...
 int8_t *dx,*dy;uint32_t*dw;CUDA_CHECK(cudaMalloc(&dx,IH*IW*4));CUDA_CHECK(cudaMalloc(&dy,hy.size()));CUDA_CHECK(cudaMalloc(&dw,hw4.size()*4));
 std::vector<int8_t> hx4(IH*IW*4,0);for(int c=0;c<IC;++c)for(int p=0;p<IH*IW;++p)hx4[p*4+c]=hx[c*IH*IW+p];
 CUDA_CHECK(cudaMemcpy(dx,hx4.data(),hx4.size(),cudaMemcpyHostToDevice));CUDA_CHECK(cudaMemcpy(dw,hw4.data(),hw4.size()*4,cudaMemcpyHostToDevice));
 int GX=(POOL_OW+PB_W-1)/PB_W,GY=(POOL_OH+PB_H-1)/PB_H;dim3 g(GX*GY,1,1);
 float ms=time_kernel([&]{v58_kernel<<<g,192>>>(dx,dw,dy,sh);},a.warmup,a.iters);
 CUDA_CHECK(cudaMemcpy(hy.data(),dy,hy.size(),cudaMemcpyDeviceToHost));print_result(a,"v58_in4ch",ms,max_abs_err(hr_n,hy));
