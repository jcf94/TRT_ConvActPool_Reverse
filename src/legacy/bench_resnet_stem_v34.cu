...
__global__ void fused_ptx_mma_oc32_group16_pool_kernel(
...
#pragma unroll
  ...
  constexpr int CHANNEL_GROUP = 16;
  constexpr int CHANNEL_GROUPS = OC_PER_BLOCK / CHANNEL_GROUP;
  for (int task = tid; task < TILE * TILE * CHANNEL_GROUPS;
       task += blockDim.x) {
    int cg = task & 1;
    int out_i = task >> 1;
    int lx = out_i % TILE;
    ...
    if (px < POOL_OW && py < POOL_OH) {
      int base_x = lx * 2;
      int base_y = ly * 2;
      int c_base = cg * CHANNEL_GROUP;
      int best[CHANNEL_GROUP] = {};
#pragma unroll
...
#pragma unroll
          for (int j = 0; j < CHANNEL_GROUP; ++j) {
            best[j] = max(best[j], int(conv_relu_tile[c_base + j][sidx]));
          }
        }
      }
...
#pragma unroll
      for (int j = 0; j < CHANNEL_GROUP; ++j) {
        ...
        y[(oc_base + c) * POOL_OH * POOL_OW + y_base] =
            static_cast<int8_t>(best[j]);
      }
    }
  }
}

template <int TILE, int WARPS, int BLOCK_THREADS>
static void run_group16_pool_case(const Args& args, const char* name,
                               const int8_t* d_x, const uint32_t* d_w_mma4,
                               int8_t* d_y, int shift,
  ...
  float ms = time_kernel([&] {
    fused_ptx_mma_oc32_group16_pool_kernel<TILE, WARPS>
        <<<grid, BLOCK_THREADS>>>(d_x, d_w_mma4, d_y, shift);
...
int main(int argc, char** argv) {
  ...
  run_group16_pool_case<4, 4, 128>(
      args, "ptx_mma_oc32_group16_pool_4x4", d_x, d_w_mma4, d_y, shift,
      h_ref, h_y);
  run_group16_pool_case<4, 2, 64>(
      args, "ptx_mma_oc32_group16_pool_4x4_w2_b64", d_x, d_w_mma4, d_y,
      shift, h_ref, h_y);
  run_group16_pool_case<4, 4, 160>(
      args, "ptx_mma_oc32_group16_pool_4x4_w4_b160", d_x, d_w_mma4, d_y,
      shift, h_ref, h_y);
  run_group16_pool_case<4, 4, 192>(
      args, "ptx_mma_oc32_group16_pool_4x4_w4_b192", d_x, d_w_mma4, d_y,
      shift, h_ref, h_y);
  run_group16_pool_case<4, 4, 224>(
      args, "ptx_mma_oc32_group16_pool_4x4_w4_b224", d_x, d_w_mma4, d_y,
      shift, h_ref, h_y);
  run_group16_pool_case<4, 4, 256>(
      args, "ptx_mma_oc32_group16_pool_4x4_w4_b256", d_x, d_w_mma4, d_y,
      shift, h_ref, h_y);
  run_group16_pool_case<4, 4, 288>(
      args, "ptx_mma_oc32_group16_pool_4x4_w4_b288", d_x, d_w_mma4, d_y,
      shift, h_ref, h_y);
  run_group16_pool_case<4, 4, 320>(
      args, "ptx_mma_oc32_group16_pool_4x4_w4_b320", d_x, d_w_mma4, d_y,
      shift, h_ref, h_y);
  run_group16_pool_case<4, 8, 192>(
      args, "ptx_mma_oc32_group16_pool_4x4_w8_b192", d_x, d_w_mma4, d_y,
      shift, h_ref, h_y);
  run_group16_pool_case<4, 8, 224>(
      args, "ptx_mma_oc32_group16_pool_4x4_w8_b224", d_x, d_w_mma4, d_y,
      shift, h_ref, h_y);
  run_group16_pool_case<4, 8, 256>(
      args, "ptx_mma_oc32_group16_pool_4x4_w8_b256", d_x, d_w_mma4, d_y,
      shift, h_ref, h_y);
  run_group16_pool_case<4, 8, 288>(
      args, "ptx_mma_oc32_group16_pool_4x4_w8_b288", d_x, d_w_mma4, d_y,
      shift, h_ref, h_y);
  run_group16_pool_case<4, 8, 320>(
      args, "ptx_mma_oc32_group16_pool_4x4_w8_b320", d_x, d_w_mma4, d_y,
      shift, h_ref, h_y);
  run_group16_pool_case<5, 4, 128>(
      args, "ptx_mma_oc32_group16_pool_5x5", d_x, d_w_mma4, d_y, shift,
      h_ref, h_y);
  run_group16_pool_case<6, 4, 128>(
      args, "ptx_mma_oc32_group16_pool_6x6", d_x, d_w_mma4, d_y, shift,
      h_ref, h_y);
  run_group16_pool_case<7, 4, 128>(
      args, "ptx_mma_oc32_group16_pool_7x7", d_x, d_w_mma4, d_y, shift,
      h_ref, h_y);
