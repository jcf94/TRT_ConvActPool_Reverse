...
template <int WARPS>
__global__ void fused_ptx_mma_oc32_pool_owner_kernel(
    const int8_t* __restrict__ x, const uint32_t* __restrict__ w_mma4,
    int8_t* __restrict__ y, int shift) {
  constexpr int OC_GROUPS = 2;
  constexpr int OC_PER_GROUP = 16;
...
#pragma unroll
    for (int wx = 0; wx < 3; ++wx) {
      int cx = px * 2 + wx - 1;
      int cy = py * 2 + wy - 1;
      bool valid_conv = cx >= 0 && cx < CONV_OW && cy >= 0 && cy < CONV_OH;
      int32_t acc[OC_GROUPS][4] = {};
...
#pragma unroll
        for (int i = 0; i < 4; ++i) {
          int v = min(max(acc[og][i] >> shift, 0), 127);
          best[og][i] = max(best[og][i], v);
        }
      }
    }
  }
...
#pragma unroll
    for (int i = 0; i < 4; ++i) {
      int row = (i < 2) ? group_id : group_id + 8;
      int out_x = lane_in_group * 2 + (i & 1);
      if (out_x < 8) {
        int oc = oc_base + og * OC_PER_GROUP + row;
        // Each lane writes two columns from the m16n8 result. All eight N
        // columns computed the same pool position, so column 0 is enough.
        if (out_x == 0) {
          y[oc * POOL_OH * POOL_OW + py * POOL_OW + px] =
              static_cast<int8_t>(best[og][i]);
        }
      }
    }
  }
...
int main(int argc, char** argv) {
  ...
  constexpr int owner_warps = 4;
  constexpr int owner_block = owner_warps * 32;
  int owner_tasks = POOL_OH * POOL_OW * (OC / 32);
  int owner_grid = (owner_tasks + owner_warps - 1) / owner_warps;
  float ptx_mma_pool_owner_ms = time_kernel([&] {
    fused_ptx_mma_oc32_pool_owner_kernel<owner_warps>
        <<<owner_grid, owner_block>>>(d_x, d_w_mma4, d_y, shift);
  }, args.warmup, args.iters);
  CUDA_CHECK(cudaMemcpy(h_y.data(), d_y, y_count, cudaMemcpyDeviceToHost));
  print_result(args, "ptx_mma_oc32_pool_owner_w4_b128",
               ptx_mma_pool_owner_ms, max_abs_err(h_ref, h_y));
