...
__device__ __forceinline__ int conv_point_valid(const int8_t* __restrict__ x,
                                                const int32_t* __restrict__ w4,
                                                int oc, int oh, int ow) {
  int acc = 0;
#pragma unroll 37
...
__device__ __forceinline__ int conv_point_auto(const int8_t* __restrict__ x,
                                               const int32_t* __restrict__ w4,
                                               int oc, int oh, int ow) {
  if (oh >= 2 && oh <= 110 && ow >= 2 && ow <= 110) {
    return conv_point_valid(x, w4, oc, oh, ow);
  }
  return conv_point(x, w4, oc, oh, ow);
}
...
__global__ void conv_only_kernel(const int8_t* __restrict__ x,
  ...
  conv[idx] = conv_point_auto(x, w4, oc, oh, ow);
}
...
__global__ void conv_relu_kernel(const int8_t* __restrict__ x,
  ...
  int oh = (idx / CONV_OW) % CONV_OH;
  int oc = idx / (CONV_OH * CONV_OW);
  conv_relu[idx] = clamp_relu_i8(conv_point_auto(x, w4, oc, oh, ow), shift);
}
...
__global__ void fused_conv_relu_pool_kernel(const int8_t* __restrict__ x,
  ...
  for (int ky = 0; ky < 3; ++ky) {
    ...
    for (int kx = 0; kx < 3; ++kx) {
      int cx = px * 2 + kx - 1;
      if (cx < 0 || cx >= CONV_OW) continue;
      best = max(best, int(clamp_relu_i8(conv_point_auto(x, w4, oc, cy, cx),
                                         shift)));
    }
  }
  y[idx] = static_cast<int8_t>(best);
...
__global__ void fused_tiled_conv_relu_pool_kernel(const int8_t* __restrict__ x,
  ...
  for (int i = tid; i < CONV_TILE * CONV_TILE; i += blockDim.x) {
    ...
    if (cx >= 0 && cx < CONV_OW && cy >= 0 && cy < CONV_OH) {
      v = clamp_relu_i8(conv_point_auto(x, w4, oc, cy, cx), shift);
    }
    conv_relu_tile[i] = v;
  }
  __syncthreads();
