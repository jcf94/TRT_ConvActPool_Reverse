...
// v36: push toward TRT CaskConvActPool. Stage all 64-OC weights into shared via
// vectorized uint4 (LDG.128) once, then reuse across the whole conv tile, raw
// int16 accumulator pool, single store. Larger TILE for more IMMA per CTA.
template <int TILE, int WARPS, int OC_GROUPS>
__global__ void fused_oc64_wshared_pool_kernel(
  ...
  __shared__ int16_t conv_acc_tile[N_TOTAL][OC_PER_BLOCK];
...
#pragma unroll
        for (int og = 0; og < OC_GROUPS; ++og) {
          ...
          if (n0 < N_TOTAL) {
            conv_acc_tile[n0][c] = static_cast<int16_t>(acc[0][og][i]);
          }
          if (has_n1 && n1 < N_TOTAL) {
            conv_acc_tile[n1][c] = static_cast<int16_t>(acc[1][og][i]);
          }
        }
      }
    }
...
#pragma unroll
          for (int c = 0; c < OC_PER_BLOCK; ++c) {
            best[c] = max(best[c], int(conv_acc_tile[sidx][c]));
          }
        }
      }
...
static void run_case(const Args& args, const char* name, const int8_t* d_x,
  ...
  float ms = time_kernel([&] {
    fused_oc64_wshared_pool_kernel<TILE, WARPS, OC_GROUPS>
...
int main(int argc, char** argv) {
  ...
  run_case<4, 8, 4, 256>(args, "v36_oc64_wshared_pool_4x4_b256", d_x, d_w_mma4,
                         d_y, shift, h_ref, h_y);
  run_case<4, 4, 4, 256>(args, "v36_oc64_wshared_pool_4x4_w4_b256", d_x,
