...
// v35: faithful TRT-direction reproduction.
// Best prior trait (v31): pool on raw int16 accumulators, dequant/ReLU once.
// TRT trait added here: one CTA owns the full 64-OC slab (OC_GROUPS=4) so every
// weight fragment is reused across the whole conv tile (tile_rows_8/cols_120
// idea), fewer CTAs, and only the pooled 56x56 int8 is written.
template <int TILE, int WARPS, int OC_GROUPS>
__global__ void fused_ptx_mma_oc64_raw_acc16_pool_kernel(
  ...
  __shared__ int16_t conv_acc_tile[OC_PER_BLOCK][N_TOTAL];
  ...
  // Stage packed int8x4 B (conv-point) tile in shared, wide reuse across OC.
...
#pragma unroll
        for (int og = 0; og < OC_GROUPS; ++og) {
          ...
          if (n0 < N_TOTAL) {
            conv_acc_tile[c][n0] = static_cast<int16_t>(acc[0][og][i]);
          }
          if (has_n1 && n1 < N_TOTAL) {
            conv_acc_tile[c][n1] = static_cast<int16_t>(acc[1][og][i]);
          }
        }
      }
    }
  ...
  // Pool max on raw int16 acc, then ReLU+shift+clamp once, single store.
...
#pragma unroll
          for (int c = 0; c < OC_PER_BLOCK; ++c) {
            best[c] = max(best[c], conv_acc_tile[c][sidx]);
          }
        }
      }
...
static void run_case(const Args& args, const char* name, const int8_t* d_x,
  ...
  dim3 grid((POOL_OW + TILE - 1) / TILE, (POOL_OH + TILE - 1) / TILE,
            OC / (OC_GROUPS * 16));
  float ms = time_kernel([&] {
    fused_ptx_mma_oc64_raw_acc16_pool_kernel<TILE, WARPS, OC_GROUPS>
...
int main(int argc, char** argv) {
  ...
  run_case<4, 8, 4, 256>(args, "ptx_mma_oc64_raw_acc16_pool_4x4_b256", d_x,
                         d_w_mma4, d_y, shift, h_ref, h_y);
  run_case<4, 4, 4, 256>(args, "ptx_mma_oc64_raw_acc16_pool_4x4_w4_b256", d_x,
                         d_w_mma4, d_y, shift, h_ref, h_y);
  run_case<6, 8, 4, 256>(args, "ptx_mma_oc64_raw_acc16_pool_6x6_b256", d_x,
                         d_w_mma4, d_y, shift, h_ref, h_y);
