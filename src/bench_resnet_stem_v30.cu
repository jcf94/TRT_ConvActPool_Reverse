...
__global__ void fused_ptx_mma_oc32_raw_acc_pool_kernel(
  ...
  __shared__ int32_t conv_acc_tile[OC_PER_BLOCK][N_TOTAL];
...
#pragma unroll
        for (int og = 0; og < OC_GROUPS; ++og) {
          ...
          if (n0 < N_TOTAL) {
            conv_acc_tile[c][n0] = acc[0][og][i];
          }
          if (has_n1 && n1 < N_TOTAL) {
            conv_acc_tile[c][n1] = acc[1][og][i];
          }
        }
      }
    }
...
static void run_case(const Args& args, const char* name, const int8_t* d_x,
  ...
  float ms = time_kernel([&] {
    fused_ptx_mma_oc32_raw_acc_pool_kernel<TILE, WARPS>
...
int main(int argc, char** argv) {
  ...
  run_case<4, 8, 256>(args, "ptx_mma_oc32_raw_acc_pool_4x4_w8_b256", d_x,
                      d_w_mma4, d_y, shift, h_ref, h_y);
  run_case<4, 4, 256>(args, "ptx_mma_oc32_raw_acc_pool_4x4_w4_b256", d_x,
                      d_w_mma4, d_y, shift, h_ref, h_y);
  run_case<4, 8, 192>(args, "ptx_mma_oc32_raw_acc_pool_4x4_w8_b192", d_x,
                      d_w_mma4, d_y, shift, h_ref, h_y);
