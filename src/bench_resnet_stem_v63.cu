...
constexpr int CB_H=5,CB_W=7,N_TILE=CB_H*CB_W,NG=(N_TILE+7)/8,OCG=4,PB_H=2,PB_W=3;
...
__global__ void v63_kernel(const int8_t* __restrict__ x,const uint32_t* __restrict__ w,int8_t* __restrict__ y,int shift){
 __shared__ int8_t patch[3*PH*PW]; __shared__ uint32_t cr[NG*8*16];
 ...
 if(threadIdx.x<32)
 #pragma unroll
 for(int ng=0;ng<NG;ng+=1){int32_t acc[OCG][4]={};
  #pragma unroll
...
int main(int argc,char**argv){Args a=parse_args(argc,argv);constexpr int sh=9;
 ...
 float ms=time_kernel([&]{v63_kernel<<<g,32>>>(dx,dw,dy,sh);},a.warmup,a.iters);
 CUDA_CHECK(cudaMemcpy(hy.data(),dy,hy.size(),cudaMemcpyDeviceToHost));print_result(a,"v63_smallN",ms,max_abs_err(hr_n,hy));
