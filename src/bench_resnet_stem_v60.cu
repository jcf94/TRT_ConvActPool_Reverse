...
__global__ void v60_kernel(const int8_t* __restrict__ x,const uint32_t* __restrict__ w,int8_t* __restrict__ y,int shift){
...
int main(int argc,char**argv){Args a=parse_args(argc,argv);constexpr int sh=9;
 ...
 float ms=time_kernel([&]{v60_kernel<<<g,32>>>(dx,dw,dy,sh);},a.warmup,a.iters);
 CUDA_CHECK(cudaMemcpy(hy.data(),dy,hy.size(),cudaMemcpyDeviceToHost));print_result(a,"v60_240static_8kb",ms,max_abs_err(hr_n,hy));
