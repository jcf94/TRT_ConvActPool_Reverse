...
// sm120 v2: 8-warp occupancy variant. faithful Blackwell (RTX 5090, sm_120) port of the sm_86 v72
...
constexpr int WARPS = 8, NG_PW = NG / WARPS;   // 3 ng/warp -> 240 IMMA total
...
__global__ void sm120_v2_kernel(const uint32_t* __restrict__ b, const uint32_t* __restrict__ w,
...
int main(int argc, char** argv) {
  ...
  float ms=time_kernel([&]{sm120_v2_kernel<<<g,WARPS*32>>>(db,dw,dy,sh);},a.warmup,a.iters);
  CUDA_CHECK(cudaMemcpy(hy.data(),dy,hy.size(),cudaMemcpyDeviceToHost));
  print_result(a,"sm120_v2_warps8",ms,max_abs_err(hr_n,hy));
