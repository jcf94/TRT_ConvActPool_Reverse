...
// sm120 v1: faithful Blackwell (RTX 5090, sm_120) port of the sm_86 v72
...
__global__ void sm120_v1_kernel(const uint32_t* __restrict__ b, const uint32_t* __restrict__ w,
...
int main(int argc, char** argv) {
  ...
  float ms=time_kernel([&]{sm120_v1_kernel<<<g,WARPS*32>>>(db,dw,dy,sh);},a.warmup,a.iters);
  CUDA_CHECK(cudaMemcpy(hy.data(),dy,hy.size(),cudaMemcpyDeviceToHost));
  print_result(a,"sm120_v1_port_v72",ms,max_abs_err(hr_n,hy));
