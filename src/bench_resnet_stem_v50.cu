...
__global__ void v50_kernel(const int8_t* __restrict__ x, const uint32_t* __restrict__ w,
...
int main(int argc, char** argv) {
  ...
  float ms=time_kernel([&]{v50_kernel<<<g,256>>>(dx,dw,dy,sh);},a.warmup,a.iters);
  CUDA_CHECK(cudaMemcpy(hy.data(),dy,hy.size(),cudaMemcpyDeviceToHost));
  print_result(a,"v50_240imma_pool_8x12",ms,max_abs_err(hr,hy));
