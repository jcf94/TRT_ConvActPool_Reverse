...
constexpr int CB_H = 9, CB_W = 13, N_TILE = CB_H * CB_W, NPAD = 120, NG = NPAD / 8, OCG = 4;
constexpr int PB_H = 4, PB_W = 6;
constexpr int KC = 8;                          // kg per k32 chunk
constexpr int NCH = K_GROUPS_MMA / KC;         // 5 chunks
constexpr int WARPS = 5, NG_PW = NG / WARPS;   // 3 ng/warp -> 240 IMMA total
...
__global__ void v70_kernel(const uint32_t* __restrict__ b, const uint32_t* __restrict__ w,
...
int main(int argc, char** argv) {
  ...
  float ms=time_kernel([&]{v70_kernel<<<g,WARPS*32>>>(db,dw,dy,sh);},a.warmup,a.iters);
  CUDA_CHECK(cudaMemcpy(hy.data(),dy,hy.size(),cudaMemcpyDeviceToHost));
  print_result(a,"v70_halo13x9",ms,max_abs_err(hr_n,hy));
