...
// v40: wide-tile S2 step toward TRT 8x120. Dynamic shared memory unlocks large
...
__global__ void fused_oc64_i8straight_kernel(
...
#pragma unroll
  ...
  if (warp_id < WARPS) {
    for (int ng0 = warp_id * 4; ng0 < N_GROUPS; ng0 += WARPS * 4) {
      int32_t acc[4][OC_GROUPS][4] = {};
#pragma unroll
...
#pragma unroll
          for (int br = 0; br < 2; ++br) {
            ...
            int n = (ng0 + p) * 8 + group_id;
            bb[p][br] = (ng0 + p < N_GROUPS && n < N_TOTAL)
                            ? smem_b4[n * K_GROUPS_MMA + kg] : 0;
          }
        }
#pragma unroll
...
#pragma unroll
          for (int p = 0; p < 4; ++p) mma_m16n8k32_s8(acc[p][og], a, bb[p]);
        }
      }
#pragma unroll
      for (int i = 0; i < 4; ++i) {
...
#pragma unroll
        for (int p = 0; p < 4; ++p) {
          int n = (ng0 + p) * 8 + lane_in_group * 2 + (i & 1);
          if (ng0 + p < N_GROUPS && n < N_TOTAL) {
#pragma unroll
            for (int og = 0; og < OC_GROUPS; ++og) {
              int c = og * OC_PER_GROUP + row;
              conv_acc[c * N_TOTAL + n] = clamp_relu_i8(acc[p][og][i], shift);
            }
          }
        }
      }
...
static void run_case(const Args& args, const char* name, const int8_t* d_x,
  ...
  auto k = fused_oc64_i8straight_kernel<TILE, WARPS, OC_GROUPS>;
...
int main(int argc, char** argv) {
  ...
  run_case<8, 8, 4, 256>(args, "v40_oc64_wide_pool_8x8_b256", d_x, d_w_mma4, d_y, shift, h_ref, h_y);
  run_case<7, 8, 4, 256>(args, "v40_oc64_wide_pool_7x7_b256", d_x, d_w_mma4, d_y, shift, h_ref, h_y);
  run_case<4, 8, 4, 256>(args, "v40_oc64_wide_pool_4x4_b256", d_x, d_w_mma4, d_y, shift, h_ref, h_y);
