...
template <int TILE, int WARPS>
__global__ void fused_ptx_mma_oc32_dual_n_accum_pool_kernel(
    const int8_t* __restrict__ x, const uint32_t* __restrict__ w_mma4,
    int8_t* __restrict__ y, int shift) {
  constexpr int OC_GROUPS = 2;
  constexpr int OC_PER_GROUP = 16;
  ...
  constexpr int N_TOTAL = CONV_TILE * CONV_TILE;
  constexpr int N_GROUPS = (N_TOTAL + 7) / 8;
  constexpr int POOL_COUNT = TILE * TILE;
  __shared__ uint32_t smem_b4[N_TOTAL][K_GROUPS_MMA];
  __shared__ int pool_accum[OC_PER_BLOCK][POOL_COUNT];
  ...
  for (int i = tid; i < OC_PER_BLOCK * POOL_COUNT; i += blockDim.x) {
    pool_accum[i / POOL_COUNT][i % POOL_COUNT] = 0;
  }
  for (int i = tid; i < N_TOTAL * K_GROUPS_MMA; i += blockDim.x) {
    int kg = i % K_GROUPS_MMA;
    int n = i / K_GROUPS_MMA;
...
#pragma unroll
        for (int which = 0; which < 2; ++which) {
          int n = which ? n1 : n0;
          if ((which == 0 || has_n1) && n < N_TOTAL) {
            int conv_x = n % CONV_TILE;
            int conv_y = n / CONV_TILE;
            int lx0 = max(0, (conv_x - 2 + 1) / 2);
            int ly0 = max(0, (conv_y - 2 + 1) / 2);
            int lx1 = min(TILE - 1, conv_x / 2);
            int ly1 = min(TILE - 1, conv_y / 2);
#pragma unroll
...
#pragma unroll
                  for (int og = 0; og < OC_GROUPS; ++og) {
                    int c = og * OC_PER_GROUP + row;
                    int v = (which ? acc[1][og][i] : acc[0][og][i]) >> shift;
                    v = min(max(v, 0), 127);
                    atomicMax(&pool_accum[c][p], v);
                  }
                }
              }
            }
  ...
  for (int i = tid; i < OC_PER_BLOCK * POOL_COUNT; i += blockDim.x) {
    ...
    if (px < POOL_OW && py < POOL_OH) {
      y[(oc_base + c) * POOL_OH * POOL_OW + py * POOL_OW + px] =
          static_cast<int8_t>(pool_accum[c][p]);
    }
  }
}
...
int main(int argc, char** argv) {
  ...
  float ptx_mma_oc32_dual_n_accum_pool_tile4_ms = time_kernel([&] {
    fused_ptx_mma_oc32_dual_n_accum_pool_kernel<4, 4>
        <<<ptx_mma_oc32_dual_n_tile4_grid, 128>>>(d_x, d_w_mma4, d_y, shift);
  }, args.warmup, args.iters);
  CUDA_CHECK(cudaMemcpy(h_y.data(), d_y, y_count, cudaMemcpyDeviceToHost));
  print_result(args, "ptx_mma_oc32_dual_n_accum_pool_4x4",
               ptx_mma_oc32_dual_n_accum_pool_tile4_ms,
               max_abs_err(h_ref, h_y));
