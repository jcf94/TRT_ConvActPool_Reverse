...
int main(int argc, char** argv) {
  ...
  dim3 oc8_tiled10_grid((POOL_OW + 9) / 10, (POOL_OH + 9) / 10, OC / 8);
  float oc8_tiled10_ms = time_kernel([&] {
    fused_tiled_oc8_conv_relu_pool_kernel<10><<<oc8_tiled10_grid, block>>>(
        d_x, d_w4, d_y, shift);
  }, args.warmup, args.iters);
  CUDA_CHECK(cudaMemcpy(h_y.data(), d_y, y_count, cudaMemcpyDeviceToHost));
  print_result(args, "fused_tiled_oc8_10x10", oc8_tiled10_ms,
               max_abs_err(h_ref, h_y));

  float oc8_tiled10_b128_ms = time_kernel([&] {
    fused_tiled_oc8_conv_relu_pool_kernel<10><<<oc8_tiled10_grid, block128>>>(
        d_x, d_w4, d_y, shift);
  }, args.warmup, args.iters);
  CUDA_CHECK(cudaMemcpy(h_y.data(), d_y, y_count, cudaMemcpyDeviceToHost));
  print_result(args, "fused_tiled_oc8_10x10_b128", oc8_tiled10_b128_ms,
               max_abs_err(h_ref, h_y));

  dim3 oc8_tiled12_grid((POOL_OW + 11) / 12, (POOL_OH + 11) / 12, OC / 8);
  float oc8_tiled12_ms = time_kernel([&] {
    fused_tiled_oc8_conv_relu_pool_kernel<12><<<oc8_tiled12_grid, block>>>(
        d_x, d_w4, d_y, shift);
  }, args.warmup, args.iters);
  CUDA_CHECK(cudaMemcpy(h_y.data(), d_y, y_count, cudaMemcpyDeviceToHost));
  print_result(args, "fused_tiled_oc8_12x12", oc8_tiled12_ms,
               max_abs_err(h_ref, h_y));

  float oc8_tiled12_b128_ms = time_kernel([&] {
    fused_tiled_oc8_conv_relu_pool_kernel<12><<<oc8_tiled12_grid, block128>>>(
        d_x, d_w4, d_y, shift);
  }, args.warmup, args.iters);
  CUDA_CHECK(cudaMemcpy(h_y.data(), d_y, y_count, cudaMemcpyDeviceToHost));
  print_result(args, "fused_tiled_oc8_12x12_b128", oc8_tiled12_b128_ms,
               max_abs_err(h_ref, h_y));
