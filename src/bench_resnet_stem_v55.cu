...
__global__ void v55_kernel(const int8_t* __restrict__ x,const uint32_t* __restrict__ w,int8_t* __restrict__ y,int shift){
 ...
 for(int ng=warp;ng<NG;ng+=4){int32_t acc[OCG][4]={};
...
int main(int argc,char**argv){Args a=parse_args(argc,argv);constexpr int sh=9;
 ...
 float ms=time_kernel([&]{v55_kernel<<<g,128>>>(dx,dw,dy,sh);},a.warmup,a.iters);
 CUDA_CHECK(cudaMemcpy(hy.data(),dy,hy.size(),cudaMemcpyDeviceToHost));print_result(a,"v55_4warp",ms,max_abs_err(hr,hy));
