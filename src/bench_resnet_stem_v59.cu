...
__global__ void v59_kernel(const int8_t* __restrict__ x, const uint32_t* __restrict__ w,
...
int main(int argc, char** argv) {
  ...
  float ms=time_kernel([&]{v59_kernel<<<g,256>>>(dx,dw,dy,sh);},a.warmup,a.iters);
  CUDA_CHECK(cudaMemcpy(hy.data(),dy,hy.size(),cudaMemcpyDeviceToHost));
  print_result(a,"v59_240static_vec",ms,max_abs_err(hr_n,hy));
