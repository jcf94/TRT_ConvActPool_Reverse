...
// v37: wide-tile S2 step toward TRT 8x120. Dynamic shared memory unlocks large
...
__global__ void fused_oc64_wide_pool_kernel(
  ...
  int16_t* conv_acc =
      reinterpret_cast<int16_t*>(smem_b4 + N_TOTAL * K_GROUPS_MMA);  // OC*N
...
#pragma unroll
        for (int og = 0; og < OC_GROUPS; ++og) {
          ...
          if (n0 < N_TOTAL) conv_acc[c * N_TOTAL + n0] = (int16_t)acc[0][og][i];
          if (has_n1 && n1 < N_TOTAL) conv_acc[c * N_TOTAL + n1] = (int16_t)acc[1][og][i];
        }
      }
    }
  }
...
#pragma unroll
      for (int c = 0; c < OC_PER_BLOCK; ++c)
        y[(oc_base + c) * POOL_OH * POOL_OW + y_base] = clamp_relu_i8(best[c], shift);
    }
  }
}
...
static void run_case(const Args& args, const char* name, const int8_t* d_x,
  ...
  size_t smem = N_TOTAL * K_GROUPS_MMA * sizeof(uint32_t) +
                OC_PER_BLOCK * N_TOTAL * sizeof(int16_t);
  auto k = fused_oc64_wide_pool_kernel<TILE, WARPS, OC_GROUPS>;
...
int main(int argc, char** argv) {
  ...
  run_case<8, 8, 4, 256>(args, "v37_oc64_wide_pool_8x8_b256", d_x, d_w_mma4, d_y, shift, h_ref, h_y);
  run_case<7, 8, 4, 256>(args, "v37_oc64_wide_pool_7x7_b256", d_x, d_w_mma4, d_y, shift, h_ref, h_y);
  run_case<4, 8, 4, 256>(args, "v37_oc64_wide_pool_4x4_b256", d_x, d_w_mma4, d_y, shift, h_ref, h_y);
