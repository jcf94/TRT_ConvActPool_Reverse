...
#pragma unroll
  ...
  for (int out_i = tid; out_i < TILE * TILE * OC_PER_BLOCK;
       out_i += blockDim.x) {
    int c = out_i % OC_PER_BLOCK;
    int spatial = out_i / OC_PER_BLOCK;
    int lx = spatial % TILE;
    int ly = spatial / TILE;
    int px = tile_px + lx;
    int py = tile_py + ly;
...
#pragma unroll
        for (int kx = 0; kx < 3; ++kx) {
          int sidx = (base_y + ky) * CONV_TILE + base_x + kx;
          best = max(best, int(conv_relu_tile[c][sidx]));
        }
      }
      int y_base = py * POOL_OW + px;
      y[c * POOL_OH * POOL_OW + y_base] = static_cast<int8_t>(best);
    }
  }
}
