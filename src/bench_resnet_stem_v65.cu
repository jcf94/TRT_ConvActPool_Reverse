...
__global__ void v65_kernel(const uint32_t* __restrict__ b, const uint32_t* __restrict__ w,
...
int main(int argc, char** argv) {
  ...
  float ms=time_kernel([&]{v65_kernel<<<g,WARPS*32>>>(db,dw,dy,sh);},a.warmup,a.iters);
  CUDA_CHECK(cudaMemcpy(hy.data(),dy,hy.size(),cudaMemcpyDeviceToHost));
  print_result(a,"v65_kstream3",ms,max_abs_err(hr_n,hy));
