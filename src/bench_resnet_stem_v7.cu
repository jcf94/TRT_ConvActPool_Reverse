...
template <int TILE>
__global__ void fused_tiled_oc16_conv_relu_pool_kernel(
    const int8_t* __restrict__ x, const int32_t* __restrict__ w4,
    int8_t* __restrict__ y, int shift) {
...
int main(int argc, char** argv) {
  ...
  dim3 oc16_tiled8_grid((POOL_OW + 7) / 8, (POOL_OH + 7) / 8, OC / 16);
  float oc16_tiled8_ms = time_kernel([&] {
    fused_tiled_oc16_conv_relu_pool_kernel<8><<<oc16_tiled8_grid, block>>>(
        d_x, d_w4, d_y, shift);
  }, args.warmup, args.iters);
  CUDA_CHECK(cudaMemcpy(h_y.data(), d_y, y_count, cudaMemcpyDeviceToHost));
  print_result(args, "fused_tiled_oc16_8x8", oc16_tiled8_ms,
               max_abs_err(h_ref, h_y));

  dim3 oc16_tiled14_grid((POOL_OW + 13) / 14, (POOL_OH + 13) / 14, OC / 16);
  float oc16_tiled14_ms = time_kernel([&] {
    fused_tiled_oc16_conv_relu_pool_kernel<14><<<oc16_tiled14_grid, block>>>(
        d_x, d_w4, d_y, shift);
  }, args.warmup, args.iters);
  CUDA_CHECK(cudaMemcpy(h_y.data(), d_y, y_count, cudaMemcpyDeviceToHost));
  print_result(args, "fused_tiled_oc16_14x14", oc16_tiled14_ms,
               max_abs_err(h_ref, h_y));
