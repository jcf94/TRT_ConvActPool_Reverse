...
__device__ __forceinline__ uint32_t max_u8x4(uint32_t a, uint32_t b) {
  int b0 = max(int(a & 0xffu), int(b & 0xffu));
  int b1 = max(int((a >> 8) & 0xffu), int((b >> 8) & 0xffu));
  int b2 = max(int((a >> 16) & 0xffu), int((b >> 16) & 0xffu));
  int b3 = max(int((a >> 24) & 0xffu), int((b >> 24) & 0xffu));
...
__global__ void fused_ptx_mma_oc16_kernel(const int8_t* __restrict__ x,
                                          const int8_t* __restrict__ w_mma,
                                          int8_t* __restrict__ y, int shift) {
  constexpr int OC_PER_BLOCK = 16;
  constexpr int CONV_TILE = TILE * 2 + 1;
  ...
  if (warp_id < WARPS) {
    for (int ng = warp_id; ng < N_GROUPS; ng += WARPS) {
      int n_start = ng * 8;
...
#pragma unroll
          for (int e = 0; e < 4; ++e) {
            int frag_idx = ar * 4 + e;
            int row = (frag_idx < 4 || (frag_idx >= 8 && frag_idx < 12))
                          ? group_id
                          : group_id + 8;
            int col = k_base + lane_in_group * 4 + (frag_idx & 3) +
                      ((frag_idx >= 8) ? 16 : 0);
            int v = 0;
            if (col < K_TOTAL) {
              v = int(w_mma[(oc_base + row) * K_PAD_MMA + col]);
            }
            vals[e] = v;
          }
          a[ar] = pack_s8x4(vals[0], vals[1], vals[2], vals[3]);
        }
...
#pragma unroll
          for (int e = 0; e < 4; ++e) {
            int frag_idx = br * 4 + e;
            int k = k_base + lane_in_group * 4 + (frag_idx & 3) +
                    ((frag_idx >= 4) ? 16 : 0);
            int n = n_start + group_id;
            int v = 0;
            if (n < N_TOTAL && k < K_TOTAL) {
...
#pragma unroll
      for (int i = 0; i < 4; ++i) {
        ...
        if (n < N_TOTAL) {
          conv_relu_tile[row][n] = clamp_relu_i8(acc[i], shift);
        }
      }
    }
  }
...
template <int TILE, int WARPS>
__global__ void fused_ptx_mma_oc32_smem_b_kernel(const int8_t* __restrict__ x,
                                                 const int8_t* __restrict__ w_mma,
                                                 int8_t* __restrict__ y,
                                                 int shift) {
  ...
  constexpr int N_TOTAL = CONV_TILE * CONV_TILE;
  constexpr int N_GROUPS = (N_TOTAL + 7) / 8;
  __shared__ int8_t smem_b[N_TOTAL][K_PAD_MMA];
  __shared__ int8_t conv_relu_tile[OC_PER_BLOCK][N_TOTAL];
  ...
  for (int i = tid; i < N_TOTAL * K_PAD_MMA; i += blockDim.x) {
    int k = i % K_PAD_MMA;
    int n = i / K_PAD_MMA;
    int v = 0;
    if (k < K_TOTAL) {
    ...
    smem_b[n][k] = static_cast<int8_t>(v);
  }
  __syncthreads();
...
#pragma unroll
          for (int e = 0; e < 4; ++e) {
            ...
            int col = k_base + lane_in_group * 4 + (frag_idx & 3) +
                      ((frag_idx >= 8) ? 16 : 0);
            vals0[e] = int(w_mma[(oc_base + row) * K_PAD_MMA + col]);
            vals1[e] =
                int(w_mma[(oc_base + OC_PER_GROUP + row) * K_PAD_MMA + col]);
          }
          a0[ar] = pack_s8x4(vals0[0], vals0[1], vals0[2], vals0[3]);
          a1[ar] = pack_s8x4(vals1[0], vals1[1], vals1[2], vals1[3]);
        }

        mma_m16n8k32_s8(acc0, a0, b);
        mma_m16n8k32_s8(acc1, a1, b);
      }
...
#pragma unroll
      for (int i = 0; i < 4; ++i) {
        ...
        int n = n_start + lane_in_group * 2 + (i & 1);
        if (n < N_TOTAL) {
          conv_relu_tile[row][n] = clamp_relu_i8(acc0[i], shift);
          conv_relu_tile[OC_PER_GROUP + row][n] =
              clamp_relu_i8(acc1[i], shift);
        }
      }
    }
  }
...
#pragma unroll
      for (int c = 0; c < OC_PER_BLOCK; ++c) {
        int global_oc = oc_base + c;
        if (global_oc < OC) {
          y[global_oc * POOL_OH * POOL_OW + y_base] =
              static_cast<int8_t>(best[c]);
        }
      }
    }
...
template <int TILE, int WARPS>
__global__ void fused_ptx_mma_oc64_smem_b_kernel(const int8_t* __restrict__ x,
                                                 const int8_t* __restrict__ w_mma,
                                                 int8_t* __restrict__ y,
                                                 int shift) {
  constexpr int OC_GROUPS = 4;
  constexpr int OC_PER_GROUP = 16;
  constexpr int OC_PER_BLOCK = OC_GROUPS * OC_PER_GROUP;
  constexpr int CONV_TILE = TILE * 2 + 1;
  constexpr int N_TOTAL = CONV_TILE * CONV_TILE;
...
#pragma unroll
          for (int e = 0; e < 4; ++e) {
            ...
            int k = k_base + lane_in_group * 4 + (frag_idx & 3) +
                    ((frag_idx >= 4) ? 16 : 0);
            int n = n_start + group_id;
            vals[e] = (n < N_TOTAL) ? int(smem_b[n][k]) : 0;
          }
          b[br] = pack_s8x4(vals[0], vals[1], vals[2], vals[3]);
        }
...
#pragma unroll
            for (int e = 0; e < 4; ++e) {
              ...
              int col = k_base + lane_in_group * 4 + (frag_idx & 3) +
                        ((frag_idx >= 8) ? 16 : 0);
              vals[e] = int(w_mma[(og * OC_PER_GROUP + row) * K_PAD_MMA + col]);
            }
            a[ar] = pack_s8x4(vals[0], vals[1], vals[2], vals[3]);
          }
...
#pragma unroll
          for (int og = 0; og < OC_GROUPS; ++og) {
            conv_relu_tile[og * OC_PER_GROUP + row][n] =
...
#pragma unroll
      for (int c = 0; c < OC_PER_BLOCK; ++c) {
        y[c * POOL_OH * POOL_OW + y_base] = static_cast<int8_t>(best[c]);
      }
    }
  }
}
...
template <int TILE, int WARPS>
__global__ void fused_ptx_mma_oc32_dual_n_smem_b_kernel(
    const int8_t* __restrict__ x, const int8_t* __restrict__ w_mma,
    int8_t* __restrict__ y, int shift) {
  constexpr int OC_GROUPS = 2;
  constexpr int OC_PER_GROUP = 16;
  constexpr int OC_PER_BLOCK = OC_GROUPS * OC_PER_GROUP;
...
#pragma unroll
          for (int e = 0; e < 4; ++e) {
            ...
            vals0[e] = (n0 < N_TOTAL) ? int(smem_b[n0][k]) : 0;
            vals1[e] = (has_n1 && n1 < N_TOTAL) ? int(smem_b[n1][k]) : 0;
          }
          b0[br] = pack_s8x4(vals0[0], vals0[1], vals0[2], vals0[3]);
          b1[br] = pack_s8x4(vals1[0], vals1[1], vals1[2], vals1[3]);
        }
...
#pragma unroll
            for (int e = 0; e < 4; ++e) {
              ...
              int col = k_base + lane_in_group * 4 + (frag_idx & 3) +
                        ((frag_idx >= 8) ? 16 : 0);
              vals[e] = int(
                  w_mma[(oc_base + og * OC_PER_GROUP + row) * K_PAD_MMA +
                        col]);
            }
            a[ar] = pack_s8x4(vals[0], vals[1], vals[2], vals[3]);
          }
...
#pragma unroll
        for (int og = 0; og < OC_GROUPS; ++og) {
          ...
          if (n0 < N_TOTAL) {
            conv_relu_tile[c][n0] = clamp_relu_i8(acc[0][og][i], shift);
          }
          if (has_n1 && n1 < N_TOTAL) {
            conv_relu_tile[c][n1] = clamp_relu_i8(acc[1][og][i], shift);
          }
        }
      }
    }
...
template <int TILE, int WARPS>
__global__ void fused_ptx_mma_oc32_dual_n_packed_b_kernel(
    const int8_t* __restrict__ x, const int8_t* __restrict__ w_mma,
    int8_t* __restrict__ y, int shift) {
  constexpr int OC_GROUPS = 2;
  constexpr int OC_PER_GROUP = 16;
...
template <int TILE, int WARPS>
__global__ void fused_ptx_mma_oc32_dual_n_packed_ab_kernel(
...
template <int TILE, int WARPS>
__global__ void fused_ptx_mma_oc32_dual_n_packed_ab_epilogue_kernel(
    const int8_t* __restrict__ x, const uint32_t* __restrict__ w_mma4,
    int8_t* __restrict__ y, int shift) {
  constexpr int OC_GROUPS = 2;
  constexpr int OC_PER_GROUP = 16;
  constexpr int OC_PER_BLOCK = OC_GROUPS * OC_PER_GROUP;
  constexpr int OC_PACKS = OC_PER_BLOCK / 4;
  constexpr int CONV_TILE = TILE * 2 + 1;
  constexpr int N_TOTAL = CONV_TILE * CONV_TILE;
  constexpr int N_GROUPS = (N_TOTAL + 7) / 8;
  __shared__ uint32_t smem_b4[N_TOTAL][K_GROUPS_MMA];
  __shared__ int8_t conv_relu_tile[OC_PER_BLOCK][N_TOTAL];
  __shared__ uint32_t conv_relu4[N_TOTAL][OC_PACKS];
...
#pragma unroll
  ...
  for (int i = tid; i < N_TOTAL * OC_PACKS; i += blockDim.x) {
    int cp = i % OC_PACKS;
    int n = i / OC_PACKS;
    int c = cp * 4;
    conv_relu4[n][cp] =
        pack_s8x4(int(conv_relu_tile[c + 0][n]), int(conv_relu_tile[c + 1][n]),
                  int(conv_relu_tile[c + 2][n]), int(conv_relu_tile[c + 3][n]));
  }
  __syncthreads();
...
#pragma unroll
        for (int kx = 0; kx < 3; ++kx) {
          int sidx = (base_y + ky) * CONV_TILE + base_x + kx;
#pragma unroll
          for (int cp = 0; cp < OC_PACKS; ++cp) {
            best4[cp] = max_u8x4(best4[cp], conv_relu4[sidx][cp]);
          }
        }
      }
      int y_base = py * POOL_OW + px;
#pragma unroll
      for (int cp = 0; cp < OC_PACKS; ++cp) {
        uint32_t v = best4[cp];
        int c = cp * 4;
        y[(oc_base + c + 0) * POOL_OH * POOL_OW + y_base] =
            static_cast<int8_t>(v & 0xffu);
        y[(oc_base + c + 1) * POOL_OH * POOL_OW + y_base] =
            static_cast<int8_t>((v >> 8) & 0xffu);
        y[(oc_base + c + 2) * POOL_OH * POOL_OW + y_base] =
            static_cast<int8_t>((v >> 16) & 0xffu);
        y[(oc_base + c + 3) * POOL_OH * POOL_OW + y_base] =
            static_cast<int8_t>((v >> 24) & 0xffu);
      }
    }
  }
}
...
static std::vector<int8_t> pack_weights_mma(const std::vector<int8_t>& w) {
  std::vector<int8_t> w_mma(OC * K_PAD_MMA, 0);
  for (int oc = 0; oc < OC; ++oc) {
    for (int k = 0; k < K_TOTAL; ++k) {
      w_mma[oc * K_PAD_MMA + k] = w[oc * K_TOTAL + k];
    }
  }
  return w_mma;
}
...
int main(int argc, char** argv) {
  ...
  std::vector<int8_t> h_w_mma = pack_weights_mma(h_w);
  ...
  CUDA_CHECK(cudaMalloc(&d_w_mma, h_w_mma.size()));
  ...
  CUDA_CHECK(cudaMemcpy(d_w_mma, h_w_mma.data(), h_w_mma.size(),
                        cudaMemcpyHostToDevice));
  ...
  dim3 oc8_tiled6_grid((POOL_OW + 5) / 6, (POOL_OH + 5) / 6, OC / 8);
  float oc8_tiled6_b128_ms = time_kernel([&] {
    fused_tiled_oc8_conv_relu_pool_kernel<6><<<oc8_tiled6_grid, block128>>>(
        d_x, d_w4, d_y, shift);
  }, args.warmup, args.iters);
  CUDA_CHECK(cudaMemcpy(h_y.data(), d_y, y_count, cudaMemcpyDeviceToHost));
  print_result(args, "fused_tiled_oc8_6x6_b128", oc8_tiled6_b128_ms,
               max_abs_err(h_ref, h_y));

  dim3 oc8_tiled7_grid((POOL_OW + 6) / 7, (POOL_OH + 6) / 7, OC / 8);
  float oc8_tiled7_b128_ms = time_kernel([&] {
    fused_tiled_oc8_conv_relu_pool_kernel<7><<<oc8_tiled7_grid, block128>>>(
        d_x, d_w4, d_y, shift);
  }, args.warmup, args.iters);
  CUDA_CHECK(cudaMemcpy(h_y.data(), d_y, y_count, cudaMemcpyDeviceToHost));
  print_result(args, "fused_tiled_oc8_7x7_b128", oc8_tiled7_b128_ms,
               max_abs_err(h_ref, h_y));

  dim3 oc8_tiled16_grid((POOL_OW + 15) / 16, (POOL_OH + 15) / 16, OC / 8);
  float oc8_tiled16_b128_ms = time_kernel([&] {
    fused_tiled_oc8_conv_relu_pool_kernel<16><<<oc8_tiled16_grid, block128>>>(
        d_x, d_w4, d_y, shift);
  }, args.warmup, args.iters);
  CUDA_CHECK(cudaMemcpy(h_y.data(), d_y, y_count, cudaMemcpyDeviceToHost));
  print_result(args, "fused_tiled_oc8_16x16_b128", oc8_tiled16_b128_ms,
               max_abs_err(h_ref, h_y));

  constexpr int block192 = 192;
  float oc8_tiled8_b192_ms = time_kernel([&] {
    fused_tiled_oc8_conv_relu_pool_kernel<8><<<oc8_tiled8_grid, block192>>>(
        d_x, d_w4, d_y, shift);
  }, args.warmup, args.iters);
  CUDA_CHECK(cudaMemcpy(h_y.data(), d_y, y_count, cudaMemcpyDeviceToHost));
  print_result(args, "fused_tiled_oc8_8x8_b192", oc8_tiled8_b192_ms,
               max_abs_err(h_ref, h_y));

  constexpr int mma_block = 256;
  dim3 ptx_mma_grid((POOL_OW + 7) / 8, (POOL_OH + 7) / 8, OC / 16);
  float ptx_mma_ms = time_kernel([&] {
    fused_ptx_mma_oc16_kernel<8, 8><<<ptx_mma_grid, mma_block>>>(
        d_x, d_w_mma, d_y, shift);
  }, args.warmup, args.iters);
  CUDA_CHECK(cudaMemcpy(h_y.data(), d_y, y_count, cudaMemcpyDeviceToHost));
  print_result(args, "ptx_mma_oc16_8x8_w8_b256", ptx_mma_ms,
               max_abs_err(h_ref, h_y));

  dim3 ptx_mma_oc32_tile6_grid((POOL_OW + 5) / 6, (POOL_OH + 5) / 6, OC / 32);
  float ptx_mma_oc32_tile6_ms = time_kernel([&] {
    fused_ptx_mma_oc32_smem_b_kernel<6, 8><<<ptx_mma_oc32_tile6_grid, mma_block>>>(
        d_x, d_w_mma, d_y, shift);
  }, args.warmup, args.iters);
  CUDA_CHECK(cudaMemcpy(h_y.data(), d_y, y_count, cudaMemcpyDeviceToHost));
  print_result(args, "ptx_mma_oc32_smem_b_6x6_w8_b256",
               ptx_mma_oc32_tile6_ms, max_abs_err(h_ref, h_y));

  dim3 ptx_mma_oc32_tile4_grid((POOL_OW + 3) / 4, (POOL_OH + 3) / 4, OC / 32);
  float ptx_mma_oc32_tile4_ms = time_kernel([&] {
    fused_ptx_mma_oc32_smem_b_kernel<4, 4><<<ptx_mma_oc32_tile4_grid, 128>>>(
        d_x, d_w_mma, d_y, shift);
  }, args.warmup, args.iters);
  CUDA_CHECK(cudaMemcpy(h_y.data(), d_y, y_count, cudaMemcpyDeviceToHost));
  print_result(args, "ptx_mma_oc32_smem_b_4x4_w4_b128",
               ptx_mma_oc32_tile4_ms, max_abs_err(h_ref, h_y));

  dim3 ptx_mma_oc32_tile5_grid((POOL_OW + 4) / 5, (POOL_OH + 4) / 5, OC / 32);
  float ptx_mma_oc32_tile5_ms = time_kernel([&] {
    fused_ptx_mma_oc32_smem_b_kernel<5, 8><<<ptx_mma_oc32_tile5_grid, mma_block>>>(
        d_x, d_w_mma, d_y, shift);
  }, args.warmup, args.iters);
  CUDA_CHECK(cudaMemcpy(h_y.data(), d_y, y_count, cudaMemcpyDeviceToHost));
  print_result(args, "ptx_mma_oc32_smem_b_5x5_w8_b256",
               ptx_mma_oc32_tile5_ms, max_abs_err(h_ref, h_y));

  dim3 ptx_mma_oc32_tile7_grid((POOL_OW + 6) / 7, (POOL_OH + 6) / 7, OC / 32);
  float ptx_mma_oc32_tile7_ms = time_kernel([&] {
    fused_ptx_mma_oc32_smem_b_kernel<7, 8><<<ptx_mma_oc32_tile7_grid, mma_block>>>(
        d_x, d_w_mma, d_y, shift);
  }, args.warmup, args.iters);
  CUDA_CHECK(cudaMemcpy(h_y.data(), d_y, y_count, cudaMemcpyDeviceToHost));
  print_result(args, "ptx_mma_oc32_smem_b_7x7_w8_b256",
               ptx_mma_oc32_tile7_ms, max_abs_err(h_ref, h_y));

  dim3 ptx_mma_oc64_tile4_grid((POOL_OW + 3) / 4, (POOL_OH + 3) / 4, 1);
  float ptx_mma_oc64_tile4_ms = time_kernel([&] {
    fused_ptx_mma_oc64_smem_b_kernel<4, 4><<<ptx_mma_oc64_tile4_grid, 128>>>(
        d_x, d_w_mma, d_y, shift);
  }, args.warmup, args.iters);
  CUDA_CHECK(cudaMemcpy(h_y.data(), d_y, y_count, cudaMemcpyDeviceToHost));
  print_result(args, "ptx_mma_oc64_smem_b_4x4_w4_b128",
               ptx_mma_oc64_tile4_ms, max_abs_err(h_ref, h_y));

  dim3 ptx_mma_oc32_dual_n_tile4_grid((POOL_OW + 3) / 4, (POOL_OH + 3) / 4,
                                      OC / 32);
  float ptx_mma_oc32_dual_n_tile4_ms = time_kernel([&] {
    fused_ptx_mma_oc32_dual_n_smem_b_kernel<4, 4>
        <<<ptx_mma_oc32_dual_n_tile4_grid, 128>>>(d_x, d_w_mma, d_y, shift);
  }, args.warmup, args.iters);
  CUDA_CHECK(cudaMemcpy(h_y.data(), d_y, y_count, cudaMemcpyDeviceToHost));
  print_result(args, "ptx_mma_oc32_dual_n_4x4_w4_b128",
               ptx_mma_oc32_dual_n_tile4_ms, max_abs_err(h_ref, h_y));

  float ptx_mma_oc32_dual_n_packed_tile4_ms = time_kernel([&] {
    fused_ptx_mma_oc32_dual_n_packed_b_kernel<4, 4>
        <<<ptx_mma_oc32_dual_n_tile4_grid, 128>>>(d_x, d_w_mma, d_y, shift);
  }, args.warmup, args.iters);
  CUDA_CHECK(cudaMemcpy(h_y.data(), d_y, y_count, cudaMemcpyDeviceToHost));
  print_result(args, "ptx_mma_oc32_dual_n_packed_b_4x4",
               ptx_mma_oc32_dual_n_packed_tile4_ms, max_abs_err(h_ref, h_y));

  float ptx_mma_oc32_dual_n_packed_ab_tile4_ms = time_kernel([&] {
    fused_ptx_mma_oc32_dual_n_packed_ab_kernel<4, 4>
        <<<ptx_mma_oc32_dual_n_tile4_grid, 128>>>(d_x, d_w_mma4, d_y, shift);
  }, args.warmup, args.iters);
  CUDA_CHECK(cudaMemcpy(h_y.data(), d_y, y_count, cudaMemcpyDeviceToHost));
  print_result(args, "ptx_mma_oc32_dual_n_packed_ab_4x4",
               ptx_mma_oc32_dual_n_packed_ab_tile4_ms,
               max_abs_err(h_ref, h_y));

  float ptx_mma_oc32_dual_n_packed_ab_epilogue_tile4_ms = time_kernel([&] {
    fused_ptx_mma_oc32_dual_n_packed_ab_epilogue_kernel<4, 4>
        <<<ptx_mma_oc32_dual_n_tile4_grid, 128>>>(d_x, d_w_mma4, d_y, shift);
  }, args.warmup, args.iters);
  CUDA_CHECK(cudaMemcpy(h_y.data(), d_y, y_count, cudaMemcpyDeviceToHost));
  print_result(args, "ptx_mma_oc32_dual_n_packed_ab_epilogue",
               ptx_mma_oc32_dual_n_packed_ab_epilogue_tile4_ms,
               max_abs_err(h_ref, h_y));
