...
__global__ void v57_kernel(const int8_t* __restrict__ x,const uint32_t* __restrict__ w,int8_t* __restrict__ y,int shift){
...
int main(int argc,char**argv){Args a=parse_args(argc,argv);constexpr int sh=9;
 ...
 float ms=time_kernel([&]{v57_kernel<<<g,192>>>(dx,dw,dy,sh);},a.warmup,a.iters);
 CUDA_CHECK(cudaMemcpy(hy.data(),dy,hy.size(),cudaMemcpyDeviceToHost));print_result(a,"v57_nhwc_vec",ms,max_abs_err(hr_n,hy));
